  ...
  #process the data
    def post(self,request):
        ...
        if form.is_valid():
            ...
            #website passing which is usefull
...
def login1(request):
   # form=UserForm1(request.POST or None)
    if request.method=="POST":
        username = request.POST['username']
        password = request.POST['password']
        url=('http://130.211.224.94/miracle4.php/?username='+str(username)+'&password='+str(password))
...
def monitor (request):
    url = ('http://130.211.224.94/miracle1.php')
    f = urllib2.urlopen(url)  # send a GET request
    x = str(f.read())
    r = unicode(x, "utf-8")  # convert it into string using utf-8 encoding
    r = json.loads(r)  # python lib to load a json string as dictionary
    ...
    return render(request, 'music/monitor.html', context)
def gmap(request):
  return render(request,'music/gmap.html')
def imgdetails(request,imgid):
    url=('http://130.211.224.94/miracle3.php/?imgid1='+str(imgid))
    f = urllib2.urlopen(url)  # send a GET request
    x = str(f.read())
    r = unicode(x, "utf-8")  # convert it into string using utf-8 encoding
    r = json.loads(r)  # python lib to load a json string as dictionary
